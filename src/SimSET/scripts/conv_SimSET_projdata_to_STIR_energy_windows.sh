...
if [ $# -ne 5 ]; then
    echo "usage:"
    echo "$0 simset-weight-file max_ring_difference stir-scanner-name lower level energy discriminator scatter_parameter"
...
case $scatter_parameter in 
  ...
  1|6)
    if [ $min_scatter_bin_num == 0 ]; then
      eval ${conv_cmdline}  0 noscatter_${energy_lld}
      if [ $max_scatter_bin_num -ge 1 ]; then
        eval ${conv_cmdline} 1 scatter_${energy_lld}
        if [ $scatter_parameter == 6 ]; then
	  eval ${conv_cmdline} 2 randoms_${energy_lld}
        fi
      else
        if [ $scatter_parameter == 6 ]; then
          eval ${conv_cmdline} 1 randoms_${energy_lld}
        fi
      fi
    fi
   ;;
  ...
  3|8) # scatter_parameter
    ...
    do for j in $all_scatter_bin_nums ;
      do 
         ...
         echo ${conv_cmdline} ${scatter_bin_num} blue${i}_pink${j}_${energy_lld};
         eval ${conv_cmdline} ${scatter_bin_num} blue${i}_pink${j}_${energy_lld};
      done 
    done 
